...
class RobotiqGripper:
    ...
    def connect(self, socket_timeout: float = 0.5) -> None:
    ...
    def open(self, speed: int, force: int) -> Tuple[bool, int]:
        return self.move(self.get_open_position(), speed=speed, force=force)

    def close(self, speed: int, force: int) -> Tuple[bool, int]:
        return self.move(self.get_closed_position(), speed=speed, force=force)

    def open_and_wait_for_pos(self, speed: int, force: int) -> Tuple[bool, int]:
        return self.move_and_wait_for_pos(self.get_open_position(), speed=speed, force=force)

    def close_and_wait_for_pos(self, speed: int, force: int) -> Tuple[bool, int]:
        return self.move_and_wait_for_pos(self.get_closed_position(), speed=speed, force=force)
    
if __name__ == "__main__":
    gripper = RobotiqGripper("172.17.139.103", 63352)
    gripper.connect()
    gripper.activate()
    import time
    gripper_pos = gripper.get_current_position()
    gripper_closed_threshold = gripper.get_closed_position() - 10
    gripper_close_pos = gripper.get_max_position()
    grasp_success = False
    ...
    for i in range(10):
        ...
        print(f"Time to close: {end - start}s")
        start = time.time()
        gripper.open_and_wait_for_pos(gripper._max_speed, gripper._max_force / 150)
        end = time.time()
        print(f"Time to open: {end - start}s")
        gripper_pos = gripper.get_current_position()
